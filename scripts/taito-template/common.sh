#!/bin/bash -e
# Common logic for airflow-template, website-template, wordpress-template,
...
if [[ $template_default_provider != "azure" ]] &&
  ...
  sed -i 's/st_bucket_name="$taito_project-$taito_env"/st_bucket_name="$taito_project-$taito_env"/' scripts/taito/project.sh
fi
