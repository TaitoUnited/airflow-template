...
function prune () {
  ...
  if ( [[ "$message" == *"[y/N]"* ]] && ! [[ "${confirm}" =~ ^[Yy]$ ]] ) || \
    ...
    sed -i "/^  airflow-template-$name:\r*\$/,/^\r*$/d" docker-compose.yaml
    sed -i "/^  airflow-template-\${taito_env}-$name:\r*\$/,/^\r*$/d" docker-compose-remote.yaml
    sed -i "/^  # airflow-template-$name:\r*\$/,/^\r*$/d" docker-compose.yaml
    sed -i "/^  # airflow-template-\${taito_env}-$name:\r*\$/,/^\r*$/d" docker-compose-remote.yaml
    if [[ -f docker-compose-cicd.yaml ]]; then
      sed -i "/^  airflow-template-$name-cicd:\r*\$/,/^\r*$/d" docker-compose-cicd.yaml
      sed -i "/^  # airflow-template-$name-cicd:\r*\$/,/^\r*$/d" docker-compose-cicd.yaml
    fi
    ...
    if [[ $name == "database" ]]; then
      ...
      if [[ ${confirm} =~ ^[Yy]$ ]]; then
        ...
        sed -i "s/airflow_template_local/${taito_project_short}local/g" \
